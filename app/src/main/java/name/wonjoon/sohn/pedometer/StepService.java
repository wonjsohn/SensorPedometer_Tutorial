...
public class StepService extends Service {
	private static final String TAG = "name.wonjoon.sohn.pedometer.StepService";
...
//    public StepDetector2 mStepDetector2;  //
//    public StepDetector4 mStepDetector2;  //
    public StepDetector_ZeroCrossing mStepDetector2;  //
    ...
    private ThresholdNotifier mThresholdNotifier;
...
//        mStepDetector2 = new StepDetector4();
        mStepDetector2 = new StepDetector_ZeroCrossing();
        ...
        mStepDetector2.addStepListener(mDistanceNotifier);
...
//        mStepDetector2.addStepListener(mCaloriesNotifier);

        mThresholdNotifier = new ThresholdNotifier(mThresholdListener, mPedometerSettings, mUtils);
        mThresholdNotifier.setThreshold(mThreshold = mState.getFloat("threshold", 0));
        mStepDetector2.addStepListener(mThresholdNotifier);
...
//        mSpeakingTimer.addListener(mCaloriesNotifier);
        mSpeakingTimer.addListener(mThresholdNotifier);
        mStepDetector2.addStepListener(mSpeakingTimer);
...
//        mStateEditor.putFloat("calories", mCalories);
        mStateEditor.putFloat("threshold", mThreshold);
        mStateEditor.commit();
...
//        if (mCaloriesNotifier != null) mCaloriesNotifier.reloadSettings();
        if (mThresholdNotifier != null) mThresholdNotifier.reloadSettings();
...
//        mCaloriesNotifier.setCalories(0);
        mThresholdNotifier.setThreshold(0);
    }

//    // update threshold at step detection.
...
//    };
    ...
    /**
     * Forwards threshold values from ThresholdNotifier to the activity.
     */
    private ThresholdNotifier.Listener mThresholdListener = new ThresholdNotifier.Listener() {
        public void valueChanged(float value) {
//            mThreshold = value;   // not a clean way to do things. consider fixing later.
            mThreshold= mStepDetector2.mAdaptiveDiff;
            passValue();
        }
        public void passValue() {
            if (mCallback != null) {
                mCallback.thresholdChanged(mThreshold);
            }
        }
    };
    ...
    private void showNotification() {
        ...
        pedometerIntent.setComponent(new ComponentName(this, SensorPedometer_Tutorial.class));
